# HOW TO INSTALL ANACONDA: https://www.youtube.com/watch?v=YJC6ldI3hWk
...
def yolo_11_extraction(img, featsave_path):
    ...
    feats = torch.mean(feat_map, dim=[2,3])  # Global average pooling
...
def feat_extractor_query():
    ...
    for queryIndex in tqdm(range(50), desc="Processing query images"):
        ...
        if not os.path.exists(query_path):
            print(f"Query image {queryPath} not found, skipping...")
            continue
        ...
        try:
            ...
            for cropIndex in range(len(crops)):
                ...
                crop_resize = cv2.resize(crop, (640, 640), interpolation=cv2.INTER_CUBIC)  # Resize to match YOLO input size
